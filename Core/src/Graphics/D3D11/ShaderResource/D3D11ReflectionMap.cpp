...
Engine::Graphics::D3D11ReflectionMap::D3D11ReflectionMap(ID3D11Device& device, char const* path)
    : _path{path}
{
...
std::shared_ptr<Engine::Graphics::D3D11ReflectionMap> Engine::Graphics::D3D11ReflectionMap::Create(ID3D11Device& device, char const* path) {
    return std::move(MakeShared<D3D11ReflectionMap>(device, path));
}
