...
Engine::Graphics::D3D11RoughnessMap::D3D11RoughnessMap(ID3D11Device& device, ID3D11DeviceContext& context, char const* path)
    : _path{path}
{
...
std::shared_ptr<Engine::Graphics::D3D11RoughnessMap> Engine::Graphics::D3D11RoughnessMap::Create(ID3D11Device& device, ID3D11DeviceContext& context, char const* path) {
    return std::move(MakeShared<D3D11RoughnessMap>(device, context, path));
}
