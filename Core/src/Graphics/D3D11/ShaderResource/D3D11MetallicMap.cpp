...
Engine::Graphics::D3D11MetallicMap::D3D11MetallicMap(ID3D11Device& device, char const* path)
    : _path{path}
{
...
std::shared_ptr<Engine::Graphics::D3D11MetallicMap> Engine::Graphics::D3D11MetallicMap::Create(ID3D11Device& device, char const* path) {
    return std::move(MakeShared<D3D11MetallicMap>(device, path));
}
