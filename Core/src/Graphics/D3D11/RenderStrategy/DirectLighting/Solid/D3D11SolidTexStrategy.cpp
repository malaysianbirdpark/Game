...
void Engine::Graphics::D3D11SolidTexStrategy::Init() {
    GRAPHICS_INFO("Textured Strategy Init()");
    _pso = D3D11PSOLibrary::ResolvePSO("solid-texture");
}

void Engine::Graphics::D3D11SolidTexStrategy::Render(ID3D11DeviceContext& context, D3D11Mesh const& mesh, D3D11Material& material) {
    mesh.Bind(context);
    _pso->Bind(context);
